    ...
    <div className="min-h-screen flex flex-col bg-ai-light relative">
      ...
      <main className="relative z-10 flex-1 max-w-5xl w-full mx-auto px-6 py-10">
        {loading ? (
          ...
          <>
            {/* ====== Profile Header Card ====== */}
            <section className="relative card p-8 md:p-10 overflow-hidden mb-6">
              {/* Glow accents */}
              ...
              <div className="relative flex flex-col md:flex-row items-center md:items-start gap-6">
                ...
                <div className="flex-1 text-center md:text-left min-w-0">
                  <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-kt-gold-700 mb-2">
                    AI Lab · Public Profile
                  </div>
                  <h1 className="text-3xl md:text-4xl font-extrabold text-kt-green-900 mb-1">
                    {profile.fullName}
                  </h1>
                  ...
                  {profile.bio && (
                    <p className="text-sm text-kt-gray-700 max-w-2xl leading-relaxed">{profile.bio}</p>
                  )}
                  {isOwnProfile && (
                    <Link
                      to="/profile"
                      className="inline-flex items-center gap-1 mt-3 text-xs font-bold text-kt-gold-700 hover:text-kt-gold-800"
                    >
                      ✎ Profilimi düzenle
                    </Link>
                  )}
              ...
              {/* Stats */}
              <div className="relative mt-6 pt-6 border-t border-kt-gray-100 grid grid-cols-3 gap-4 text-center">
                <Stat label="Onaylı proje" value={profile.stats.projectCount} tone="cyan" />
                <Stat label="Toplam beğeni" value={profile.stats.totalLikes} tone="gold" />
                <Stat label="Toplam yorum" value={profile.stats.totalComments} tone="violet" />
              </div>
            </section>
              ...
              {profile.projects.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {profile.projects.map((p) => (
                    <article
                      key={p.id}
                      className={`card-hover p-5 ${p.isHighlight ? 'ring-2 ring-kt-gold-400' : ''}`}
                    >
                      ...
                      <h3 className="text-lg font-bold text-kt-green-900 mb-1.5 line-clamp-2">
                        {p.projectName}
                      </h3>
                      <p className="text-sm text-kt-gray-600 line-clamp-3 mb-3">
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        {p.technologies.slice(0, 5).map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded text-[11px] font-semibold bg-kt-green-50 text-kt-green-800"
                          >
                            {t}
                          </span>
                        ))}
                        {p.technologies.length > 5 && (
                          <span className="px-2 py-0.5 rounded text-[11px] text-kt-gray-400">
                            +{p.technologies.length - 5}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center justify-between pt-3 border-t border-kt-gray-100 text-xs">
                        <span className="text-kt-gray-500">{fmtDateRange(p.startDate, p.endDate)}</span>
                        <div className="flex items-center gap-3">
                          ...
                          <span className="flex items-center gap-1 text-kt-gold-700 font-semibold">
...
function Stat({ label, value, tone }: { label: string; value: number; tone: 'cyan' | 'gold' | 'violet' }) {
  ...
  return (
    <div>
      ...
      <div className="text-[11px] uppercase tracking-wider text-kt-gray-500 font-semibold mt-0.5">{label}</div>
    </div>
  );
}
