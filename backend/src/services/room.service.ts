...
import { listRoomAppointments } from './appointment.service';
...
export interface RoomAvailabilityDto {
  ...
  /** [from, to] aralığında dolu randevu saatleri — gün gün (PII'siz). */
...
export async function getRoomAvailability(
  ...
  // Dolu randevu saatleri (PII'siz) — gün gün grupla. Tarih kovası YEREL güne
  // göre (ymdLocal) hesaplanır: start_at UTC ISO'dur; UTC slice(0,10) TR'de
  // 00:00-03:00 randevularını bir önceki güne atıp gün başlığı ile saat etiketini
  // çelişkiye düşürürdü. Frontend saatleri zaten yerel gösterdiği için kova da
  // yerel olmalı.
  const appts = await listRoomAppointments(roomId, from, `${to}T23:59:59.999`);
  const byDate = new Map<string, Array<{ start: string; end: string }>>();
  for (const a of appts) {
    const date = ymdLocal(new Date(a.startAt));
    const list = byDate.get(date) ?? [];
    list.push({ start: a.startAt, end: a.endAt });
    byDate.set(date, list);
  }
  const appointments: RoomAvailabilityDay[] = [...byDate.entries()]
